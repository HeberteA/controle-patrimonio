...
# Usa os segredos configurados em st.secrets para conectar
conn = st.connection("gspread", type=GSheetsConnection)

# --- Carregar Dados Existentes ---
# Tenta carregar a planilha. Se estiver vazia, cria um DataFrame vazio.
try:
...
# --- Função para Gerar Número de Tombamento Único ---
    ...
    if submitted:
        if nome_produto and local_responsavel and num_nota_fiscal:
            ...
            if novo_tombamento is not None:
                # Cria um DataFrame para o novo item
                ...
                # Concatena os dados existentes com o novo item
                updated_df = pd.concat([existing_data, novo_item_df], ignore_index=True)

                # Atualiza a planilha do Google Sheets com o DataFrame completo
